  ...
  async execute(interaction) {
    ...
    if (categoryFilter) {
      ...
      collector.on('collect', async i => {
        ...
        const categoryCollector = i.message.createMessageComponentCollector({ time: 300000 });
        
        categoryCollector.on('collect', async ci => {
          if (ci.user.id !== interaction.user.id) {
            return ci.reply({ content: 'These buttons are not for you!', ephemeral: true });
          }
          ...
          if (ci.customId === 'help_back') {
            try {
              ...
              if (error.code !== 10062) console.error('Error updating help interaction:', error);
            }
